...
const buildVideoTrack = (defaultClient, DeveloperKey, api) => {
  ...
  titleAsset.setStyle("minimal").setText("Hello World");

  let title = new Shotstack.Clip();
  title.setAsset(titleAsset).setStart(0).setLength(5).setEffect("zoomIn");

  let track = new Shotstack.Track();
  track.setClips([title]);
...
const addVideoToQueueFun = (req, res, next) => {
  ...
  buildVideoTrack(defaultClient, DeveloperKey, api);
};

router.use(addVideoToQueueFun);
